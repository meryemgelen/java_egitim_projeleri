...
public class Test9Level2Cache {
	...
	public static void main(String[] args) {
		...
		personel2 = session.get(Personel.class, 4);
		System.out.println(personel2);
		session.close();
		 ...
		 * 
		 * Native sorgular cache'lenmez. session.createSQLQuery("select * from Foo")
		 * .setHint("org.hibernate.cacheable", true) .getResultList();
		 * 
		 * HQL sorgular da cache'lenmez. session.createQuery("select f from Foo f")
		 * .setHint("org.hibernate.cacheable", true) .getResultList();
		 * 
		 * Native query ile update yapýldýðýna neyin güncellendiðini hibernate
		 * bilemediðinden tüm cache silinir.
		 * 
		 * nativeQuery = session.createNativeQuery("update FOO set ... where ...");
		 * nativeQuery.unwrap(org.hibernate.SQLQuery.class).addSynchronizedEntityClass(
		 * Foo.class); nativeQuery.executeUpdate();
		 * 
		 * aþaðýdaki ayar ise cache özelliðini tüm entitiy'ler için otomatik olarak
		 * açar. Entity sýnýflarýna @Cache yazmak zorunda kalmayýz ama kesinlikle
		 * tavsiye edilmez. <property
		 * name="javax.persistence.sharedCache.mode">DISABLE_SELECTIVE</property>
		 * 
		 */
		// @formatter:on
