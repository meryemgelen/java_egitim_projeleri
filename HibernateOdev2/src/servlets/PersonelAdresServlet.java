...
public class PersonelAdresServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<PersonelAdres> personelAdresler = new PersonelAdresBusiness().getPersonelAdresler(id);

		JSONObject cevap = new JSONObject();
		PersonelAdresModel[] adresArr = new PersonelAdresModel[personelAdresler.size()];

		for (int i = 0; i < personelAdresler.size(); i++) {

			PersonelAdresModel model = new PersonelAdresModel();
			model.setId(personelAdresler.get(i).getId());
			model.setAdres(personelAdresler.get(i).getAdres());
			adresArr[i] = model;
